...
Rebass Native components use [styled-system][system] for responsive, theme-based style props.

## Margin and Padding

All Rebass Native component use the [space][space] utility from [styled-system][system] to handle responsive margin and padding props based on a global spacing scale (`theme.space`).
...
All Rebass Native components use [styled-system's][system] [color][color] function to add the `color` and `bg` props.
...
Many Rebass Native props accept arrays as values to set mobile-first responsive styles.
